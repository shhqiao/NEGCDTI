...
def read_data(data_folder,drug_sim_path,target_sim_path,DTI_path):
    SR = pd.read_csv(drug_sim_path,header=None).values
    SD = pd.read_csv(target_sim_path,header=None).values
...
def data_loader(smile_name, pro_name):
    smiles_feature = torch.LongTensor(np.load(smile_name))
    pro_feature = torch.LongTensor(np.load(pro_name))
    return smiles_feature, pro_feature
